...
<div class='base_center'>
    ...
    <? foreach ($standard_stations as $station){ temp0043_station($station,$last_day_rates);}?>
    <tr>
        <td colspan=12>
            <div class='station_type_head'> 
    ...
    <? foreach ($saving_stations as $station){ temp0043_station($station,$last_day_rates);}?>
    <tr>
        <td colspan=12>
            <div class='station_type_head'> 
    ...
    <? foreach ($common_stations as $station){ temp0043_station($station,$last_day_rates);}?>
    <tr>
        <td colspan=12>
            <?= $common_pagination?>
        </td>
    ...
    <?php function temp0043_station($station,$last_day_rates){?>

        <? 
        $rate = null;
        if($station['station_type']==ESC_STATION_TYPE_SAVING){
            $rate = isset($last_day_rates[$station['id']])?$last_day_rates[$station['id']]:null;
        }else if($station['station_type']==ESC_STATION_TYPE_COMMON){
            $rate = $last_day_rates['rate'];
        }?>    
        <tr>
            ...
            <td style="width:100px"> 
                <?= $station['last_day_energy']?$station['last_day_energy']."度":""?>
            </td>
            <td style="width:100px"> 
                <?= ($rate&&$station['alive']== 1&&$station['last_day_energy']>0.0)?$rate."%":""?>
            </td>
            <td style="width:100px"> 
                <font style="font-size:14px;font-weight:bold;color:#063;">
                <?= ($rate>0&&$rate<100&&$station['last_day_energy'])? h_round2($station['last_day_energy']*$rate/(100-$rate))."度":""?>
                </font>
            </td>
