...
类型:<?= h_bug_type_select($this->input->get('type'))?>
...
<div class='operate'>
    ...
    <? if(isset($station) && $station){ ?>
        <a href="/backend/bug?station_id=<?= $station['id'] ?>" class="btn btn-primary">清除查询</a>
    <?}else{?>
        <a href="/backend/bug" class="btn btn-primary">清除查询</a>
    <?}?>
</div>
</form>

<? if($station && $station['alive'] == ESC_OFFLINE){?>
<div style="width:984px;line-height:24px; padding:2px 8px;background-color:<?= $dis_bugs?"#eaa":"#aea"?>">
    <? if($dis_bugs){?>
...
<td> <?= $bug['id']?> </td>
<td> 
    <a href="/backend/bug?station_id=<?= $bug['station']['id']?>">
        <?= $bug['station']['name_chn'] ?> 
    </a>
</td>
