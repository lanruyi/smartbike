...
class CIUnit_TestCase extends PHPUnit_Framework_TestCase
{
	...
	*/
	protected function load_fixt_a($fixts)
	{
	...
	*/
	protected function load_fixt($fixts)
	{
		foreach ( $fixts as $fixt )
		{
			...
			if (file_exists(TESTSPATH . 'fixtures/' . $fixt . '_fixt.php')) {
                include(TESTSPATH . 'fixtures/' . $fixt . '_fixt.php');
                $this->$fixt_name = $d;
            }			
			else
			{
				die('The file '. TESTSPATH . 'fixtures/' . $fixt . '_fixt.php doesn\'t exist.');
			}
		}
	}
}
