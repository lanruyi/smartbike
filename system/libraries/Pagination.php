...
class CI_Pagination {
	...
	function create_links()
	{
		...
		if ($num_pages == 1)
		{
            $info = "&nbsp; 第 1 - ".$this->total_rows." 个  共有 ".$this->total_rows." 个";
		    $output = $this->full_tag_open."<li>".$info."</li>".$this->full_tag_close;
			return $output;
		}
        ...
        $_first = ($this->cur_page-1)*$this->per_page+1;
        $_last  = $this->cur_page*$this->per_page > $this->total_rows ? $this->total_rows:$this->cur_page*$this->per_page;
        if($this->has_tail){
            $info = "<li>"."&nbsp; 第 ".$_first." - ".$_last." 个 共有 ".$this->total_rows." 个"."</li>";
        }else{
            $info = "";
        }
		$output = $this->full_tag_open.$output.$info.$this->full_tag_close;
