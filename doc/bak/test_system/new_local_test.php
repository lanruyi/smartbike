...
function getOutput($url) {
    ...
    curl_setopt($ch, CURLOPT_COOKIEFILE, dirname(__FILE__) . "/local_cookie.txt");
...
function init_php_url() {
    ...
    curl_setopt($ch, CURLOPT_COOKIEJAR, dirname(__FILE__) . "/local_cookie.txt");
...
function generate_test_url($host) {
    $test_urls = array(
        ...
        $host . "/backend/statistics?station_id=17",
        $host . "/backend/restart?station_id=17",
        $host . "/backend/station/mod_station/17",
        $host . "/backend/esgconf/read_setting/17",
        $host . "/backend/esgconf/set_setting/17",
        $host . "/backend/station/update_rom/17",
