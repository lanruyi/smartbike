...
<div class='es_sub2_menu' style="border-bottom:4px solid #ccc">  
    ...
    <? foreach($cities as $city){?>
        ...
        <a  href="/frontend/stations/category/<?=  $city->getId() ?>"> <?= $city->getNameChn()?> </a> </li>
        <li class="divider-vertical"></li>
    <? }?>
        <li style="float:right"> 
            图例: 
                  <? if($this->current_project->getType() == ESC_PROJECT_TYPE_STANDARD_SAVING_COMMON){?>
                    <font style="background-color:#888;color:#fff">[基]-基准站</font> 
                    <font style="background-color:#aaa;color:#fff">[标]-标杆节能站</font> 
                    <font>[普]-普通节能站</font>
                  <? }?>
                  <font style="font-weight:bold;color:#900">[安装]-正在安装</font> </li>
    </ul>
</div>
</div>
...
.es_cc_body ul li span.saving_p{ color:#006; float:right}
</style>
            ...
            <div class='es_cc_body' id=es_cc_body_<?= $total_load."_".$build?>>
                ...
                <ul>
                 ...
                 <? foreach($stations['6p1'] as $key=>$station){ ?>
                   <li>
                      <?= in_page_station_item($station) ?></li>
                 <?}?>
                </ul>
            </div>
        </div>
